...
// FIX: Typed API response shape so we never need `any`
...
export default function ContactsAdmin() {
  ...
  async function fetchContacts() {
    try {
      ...
      // FIX: Parse as the known union of possible shapes, then normalize below
      const data: ContactInquiry[] | ContactsApiResponse = await res.json()
  ...
  // FIX: `contacts` is now always ContactInquiry[], so no `any` cast needed
  ...
  if (loading) {
    return (
        <div className="flex min-h-screen bg-slate-50">
          ...
          <div className={`flex-1 flex items-center justify-center ${isDesktop ? "ml-0" : "ml-72"}`}>
  ...
  return (
      <div className="flex min-h-screen bg-slate-50">
        ...
        <div className={`flex-1 min-w-0 ${isDesktop ? "ml-0" : "ml-72"}`}>
          ...
          <main className="mt-18 lg:mt-0 p-4 md:p-8 max-w-7xl mx-auto space-y-6">
