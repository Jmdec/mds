        ...
        <div className="space-y-0">
          ...
          {[
            ...
            { icon: <Clock size={14} />, label: "Time", val: timeStr },
            {
              ...
              label: "Service",
              val: booking.notes?.replace("Service: ", "") ?? "—",
            },
          ].map(({ icon, label, val }) => (
            <div
              key={label}
...
export default function AdminBookingsTable() {
  ...
  }, [currentPage, perPage, search, statusFilter, token]);
        ...
        <div className="bg-white border border-blue-100 rounded-2xl shadow-sm p-5 mb-5 flex flex-wrap items-center gap-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-sm text-slate-500">
              Admin Dashboard · All patient appointments
            </p>
          </div>
          <div className="flex gap-3 flex-wrap">
            {[
              {
                label: "Total",
                val: meta?.total ?? safeBookings.length,
              ...
              {
                label: "Page",
                val: `${meta?.current_page ?? 1}/${totalPages}`,
                color: "bg-blue-50 text-blue-700 border-blue-200",
              },
            ].map((s) => (
              <div
                key={s.label}
                className={`flex flex-col items-center px-3 py-1.5 rounded-xl border text-xs font-medium ${s.color}`}
              >
                ...
                {isLoading ? (
                  ...
                  safeBookings.map((b, idx) => {
                    ...
                    return (
                      <tr
                        key={b.id ?? idx}
                        className="border-b border-slate-100 hover:bg-[#f9fbff] transition-colors group"
                      >
